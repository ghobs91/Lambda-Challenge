...
function longestString(strs) {
  ...
  for (let i = 0; i<strs.length; i++){
    const stringCandidate = strs[i];
    if (stringCandidate.length > longest.length)
    longest = stringCandidate;
  }
  return longest;
}
...
function computeUserAverageAge(users) {
  ...
  for (let i = 0; i < users.length; i++) {
    const computerScientist = users[i];
    ageSum += computerScientist.age;
  }
